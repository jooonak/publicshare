function PageMaker(param){
    ...
    function makePage() {
        ...
        if (prev) {
            str += tag.replace("%page", '<a class="btn" href="' + url + '?page=' + (startPage - 1) + '">Prev</a>');
        } else {
			str += tag.replace("%page", '<a class="btn" disabled>Prev</a>');
		}

        for (var i = startPage; i <= endPage; i++){
            str += tag.replace("%page", '<a class="btn" href="' + url + '?page=' + (i) + '">' + (i) + '</a>');
        }

        if (next) {
            str += tag.replace("%page", '<a class="btn" href="' + url + '?page=' + (endPage + 1) + '">Next</a>');
        }
        return str;
    }
    return makePage();
